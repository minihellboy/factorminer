...
def export_formulas_qlib(library: FactorLibrary, path: str | Path) -> None:
    ...
    logger.info(
        ...
        library.size, path, n_translatable,
    )
...
def _anonymized_row(f: Factor) -> dict:
    """Build one redacted row: no raw formula, only its hash plus metrics."""
    formula_hash = hashlib.sha256(f.formula.encode("utf-8")).hexdigest()[:16]
    return {
        "factor_id": f.id,
        "name": f.name,
        "family": infer_family(f.formula),
        "formula_hash": formula_hash,
        "ic_mean": f.ic_mean,
        "ic_paper_mean": float(f.ic_paper_mean or 0.0),
        "ic_abs_mean": float(f.ic_abs_mean or 0.0),
        "icir": f.icir,
        "ic_paper_icir": float(f.ic_paper_icir or 0.0),
        "ic_win_rate": f.ic_win_rate,
        "max_correlation": f.max_correlation,
        "batch_number": f.batch_number,
        "admission_date": f.admission_date,
    }


def export_anonymized(library: FactorLibrary, path: str | Path, *, fmt: str = "csv") -> None:
    """Export a redacted factor table for sharing outside a trusted boundary.

    Same row shape as :func:`export_csv`, but the raw ``formula`` column is
    dropped entirely and replaced with ``formula_hash`` (first 16 hex chars
    of ``sha256(formula)``) plus a ``family`` label from
    :func:`factorminer.architecture.families.infer_family`. All numeric
    metrics (IC, ICIR, win rate, correlation, batch, admission date) are
    kept unchanged. Identical formulas always hash identically, so
    duplicate-formula detection still works downstream without ever
    exposing the formula text itself.
    ...
    path : str or Path
    fmt : {"csv", "json"}
        Output container format. Row content is identical either way.
    """
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    rows = [_anonymized_row(f) for f in library.list_factors()]

    if fmt == "csv":
        with open(path, "w", newline="") as fp:
            writer = csv.DictWriter(fp, fieldnames=_ANONYMIZED_FIELDNAMES)
            writer.writeheader()
            for row in rows:
                writer.writerow(row)
    elif fmt == "json":
